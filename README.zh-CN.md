...
**v1.1：** GIF 和竖屏短视频制作、自定义画面区域、6款内置韩文字体及自由文字配色。
...
<p align="center">
  <img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v1.0.0/docs/screenshots/v1.0.0/04-home.png" width="1000" alt="右下角固定显示鲜紫色KAI和绿色NAVER小徽标的新播放器">
</p>
...
<p align="center"><img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v1.1.0/docs/screenshots/v1.1.0/01-shorts-styles.png" width="980" alt="实际短视频编辑界面：竖屏画面预览、拖动选区、韩文文字叠加和构图设置"></p>

1. 拖动时间轴两端设置开始和结束位置，拖动中间可移动整个片段。使用 **장면 미리보기**（画面预览）滑块查看选区内的画面。
2. 在 **구간·화면**（片段与画面）中点击 **영역 직접 선택**（选择区域），在原画面上拖出矩形。拖动内部可移动，拖动边缘或角点可调整大小。选定后默认显示整个选区。使用 **100–400%缩放**、预览拖动和鼠标滚轮调整构图；重置可恢复原画面。同一区域和构图会应用于整个所选片段。
3. 在 **문구·스타일**（文字与样式）中输入最多 **160字、4行**。内置 Pretendard、Black Han Sans、Do Hyeon、Jua、Nanum Myeongjo、Gaegu 六款韩文字体，无需另行安装，可按 OFL 许可用于商业视频，附带原始许可。支持 **18色调色板、自定义颜色对话框及HEX输入**，可调整字号和深色背景，并拖动文字定位。
4. 点击 **쇼츠 저장**（保存短视频），导出 **9:16、1080×1920 / 720×1280、30fps 的 H.264 MP4**。取消 **원본 소리 포함** 可导出无声视频。

**保存完成后，底部会持续显示荧光青柠色的“✓ 쇼츠 저장 완료”（短视频保存完成）。** 可查看文件名、时长和大小，并点击 **완성된 영상 열기**（打开成品视频）或 **저장 폴더**（保存文件夹）。更改选区、文字或构图后，状态变为 **설정 변경 · 다시 저장 필요**（设置已更改，需要重新保存），明确区分之前的文件。
...
**[查看此编辑器实际导出的 MP4](https://github.com/ai-campus-kr/shin-player/releases/download/v1.1.0/ShinPlayer-shorts-demo.mp4)** · 截图和成品使用原创合成演示视频。

## 把精彩片段变成 GIF

在本地播放器中打开视频文件，点击 **GIF**，**拖动时间轴两端的手柄**选择片段，再点击 **GIF 만들기**（制作 GIF）。拖动中间的选中区域，可在保持时长的同时移动整个片段。

**荧光青柠色的“✓ GIF 저장 완료”（GIF 保存完成）** 和 **완성된 GIF 열기**（打开成品 GIF）按钮让完成状态一目了然。完成区域固定在滚动区域之外，不会自动消失。保存中、取消、失败和设置更改均有独立状态。

- 使用 **선택 확대**（放大选区）和 **전체 보기**（查看全片）精细调整短片段。选中手柄后，方向键每次调整0.1秒，配合Shift为1秒，配合Ctrl为10秒。
- 仍可直接输入开始、结束时间，或使用 **현재 위치**（当前位置），这些操作与时间轴双向同步。

- 可直接输入 **0.25–100 倍速**或选择预设，实时显示预计长度：**原视频 10 分钟 ÷ 10 倍速 → 1 分钟 GIF**。
...
<p align="center"><img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v1.0.0/docs/screenshots/v1.0.0/02-gif-completed.png" width="620" alt="实际 GIF 界面：拖动手柄选择区间、移动选区、放大与时间输入同步"></p>

*使用合成测试视频拍摄的实际应用界面。*

## 快速开始

1. 从[最新发行版](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.1.0)下载 `ShinPlayer-版本-win-x64.zip`，并完整解压。
...
**“解释这段内容” → 根据字幕回答。“跳到那里” → 移动播放位置。** [下载 v1.1](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.1.0)
...
**验证范围：** 98 项集成检查涵盖密钥保存与连接状态、字幕自动加载、无字幕与视频切换、聊天布局及本地 GIF。真实 YouTube 字幕自动加载、GPT-5.4-mini 回答及时间跳转已在此前的测试版开发中另行验证。v1.1 回归检查使用模拟 API 响应和隔离的 WebView2 页面。字幕可用性和网站变化可能影响其他视频。
...
v1.1 使用 WPF/libmpv 和隔离 WebView2 的 **98 项集成检查**验证。API 使用模拟响应，真实服务验证范围见上文。[更新日志](CHANGELOG.md)
...
共运行 98 项集成检查。定位检查覆盖两种窗口大小和播放列表状态下的 28 个位置，以及布局刷新前的重复输入、释放位置、鼠标捕获丢失、文件切换和全屏。也检查 1–2 倍 WPF 布局变换，但未自动化物理鼠标输入或实际显示器 DPI 变更。界面检查渲染四种风格的启动画面、最小窗口、播放列表和截图窗口，检查按钮重叠及字幕选择显示。还检查设置序列化、旧设置兼容性、选择按钮，以及播放时切换风格对位置和速度的保持。
...
[한국AI교육진흥원](https://github.com/ai-campus-kr) · [最新发行版](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.1.0) · [报告问题或提出功能建议](https://github.com/ai-campus-kr/shin-player/issues)
