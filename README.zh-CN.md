...
**[下载最新版本](https://github.com/ai-campus-kr/shin-player/releases/latest)**　·　[试用 6 秒演示视频](https://github.com/ai-campus-kr/shin-player/releases/download/v0.5.0/ShinPlayer-subtitle-demo.mp4)　·　[查看版本介绍](https://github.com/ai-campus-kr/shin-player/releases/tag/v0.5.0)
...
1. 从[最新发行版](https://github.com/ai-campus-kr/shin-player/releases/latest)下载 `ShinPlayer-版本-win-x64.zip`，并完整解压。
...
**音量增强和字幕截图均在本机处理，不需要 GPT/API 密钥，也不会产生 Token 费用。** 视频和字幕不会上传至服务器。

当前版本**没有 ChatGPT API 密钥输入框，也没有 AI 分析功能**。计划中的接入条件如下。

<details>
<summary><strong>AI 功能开发条件——尚未实现</strong></summary>

- 计划使用的 OpenAI API 模型为 **`gpt-5.4-mini`**。
- 仅在能够从视频文件本身提取**内嵌文本字幕**时，才启用 AI 搜索和摘要。即使播放时关闭了字幕显示，只要有受支持的内嵌字幕，仍可作为处理对象。
- 不处理外部 SRT、自动加载的外部字幕、内嵌图像字幕或烧录在画面中的文字。
- 没有受支持的字幕或提取失败时，会禁用 AI 功能并说明原因。不会调用 OpenAI API，也不会改用语音转写或 OCR。
- 这些条件仅适用于未来的 AI 功能。普通播放和现有字幕显示功能不受影响。

</details>

## 开发与验证

使用 C# / .NET 8 WPF 和 mpv 构建的 Windows 原生应用，运行时无需浏览器或开发服务器。

0.5.0 已通过实际 WPF/libmpv 环境中的 **61 项集成检查**，并验证了从发行 ZIP 安装的可执行文件。变更和已知问题请参阅[更新日志](CHANGELOG.md)。更新日志及图片来源详情目前为韩语。
...
共运行 61 项集成检查。定位检查覆盖两种窗口大小和播放列表状态下的 28 个位置，以及布局刷新前的重复输入、释放位置、鼠标捕获丢失、文件切换和全屏。也检查 1–2 倍 WPF 布局变换，但未自动化物理鼠标输入或实际显示器 DPI 变更。界面检查渲染四种风格的启动画面、最小窗口、播放列表和截图窗口，检查按钮重叠及字幕选择显示。还检查设置序列化、旧设置兼容性、选择按钮，以及播放时切换风格对位置和速度的保持。
...
[한국AI교육진흥원](https://github.com/ai-campus-kr) · [最新发行版](https://github.com/ai-campus-kr/shin-player/releases/latest) · [报告问题或提出功能建议](https://github.com/ai-campus-kr/shin-player/issues)
