...
foreach ($name in @('README.md','LICENSE','THIRD-PARTY-NOTICES.md','CHANGELOG.md')) { Copy-Item -LiteralPath (Join-Path $projectRoot $name) -Destination $publishPath -Force }
...
foreach ($name in @('README.md','LICENSE','THIRD-PARTY-NOTICES.md','CHANGELOG.md','.gitignore')) { Copy-Item -LiteralPath (Join-Path $projectRoot $name) -Destination $sourcePath -Force }
