...
로컬 영상도, 유튜브도 **GIF** 버튼으로 움직이는 짤을 만드세요. 시작·끝 시간을 입력하거나 **현재 위치**로 지정하고, **GIF 만들기**를 누르면 됩니다.

- **0.2~30초**, 최대 크기 **360 / 480 / 720px**, **10 / 12 / 15 / 20fps**. 무음으로 반복 재생됩니다.
...
<p align="center"><img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v0.6.0-beta.3/docs/screenshots/v0.6.0-beta.3/01-gif-dialog.png" width="620" alt="신플레이어 GIF 만들기: 시작·끝 시간, 크기, 프레임 선택과 실제 저장 완료 화면"></p>

*합성 테스트 영상으로 촬영한 실제 앱 화면입니다.*

## 빠른 시작

1. [최신 릴리스](https://github.com/ai-campus-kr/shin-player/releases/tag/v0.6.0-beta.3)에서 `ShinPlayer-버전-win-x64.zip`을 받고 압축을 모두 풉니다.
...
**“이 내용 설명해줘” → 스크립트로 답변. “그 부분으로 이동해줘” → 재생 위치 이동.** [베타 다운로드](https://github.com/ai-campus-kr/shin-player/releases/tag/v0.6.0-beta.3)
...
3. 패널의 **API 설정**에 본인 OpenAI API 키를 저장하고 질문합니다. 일반 질문은 재생 위치를 유지합니다. **“그 부분으로 이동해줘”**처럼 이동을 요청하거나 근거 시간 버튼을 누르면 해당 구간으로 이동합니다.
...
<sub>실제 유튜브 영상과 GPT-5.4-mini API로 검증한 주소창·채팅 패널 화면입니다.</sub>
...
**베타 검증 범위:** 81개 통합 검사와 실제 `gpt-5.4-mini` API 검사를 통과했습니다. 실제 유튜브 영상의 한국어 스크립트 301개를 읽고, 일반 질문은 재생 위치를 유지하며 “방금 설명한 부분으로 이동해줘”는 10:45로 이동하는 것을 확인했습니다. 직전 근거로 이동할 때는 추가 API 호출이 없습니다. 자막 제공 여부·광고·로그인·사이트 변경에 따라 사용할 수 없는 영상도 있습니다.
...
0.6 베타는 WPF/libmpv와 격리된 WebView2를 사용하는 **81개 통합 검사**로 검증합니다. API 검사는 대체 응답을 사용하며 실제 서비스 검증 한계는 위에 명시했습니다. [변경 기록](CHANGELOG.md)
...
총 81개 통합 검사를 실행합니다. 탐색 좌표는 두 창 크기와 재생목록 상태별 28개 지점, 레이아웃 갱신 전 반복 입력, 놓기 위치, 캡처 해제, 파일 전환과 전체화면을 포함합니다. 1~2배 WPF 레이아웃 변환도 검사하지만 물리적인 마우스 입력이나 실제 모니터 DPI 변경을 자동화한 검사는 아닙니다. UI는 네 가지 스타일의 시작 화면·최소 창·재생목록·캡처 창을 렌더하고 버튼 겹침과 자막 선택 표시를 확인합니다. 설정 직렬화와 이전 설정 호환, 선택 버튼, 재생 중 스타일 전환과 위치·배속 유지도 검사합니다.
...
[한국AI교육진흥원](https://github.com/ai-campus-kr) · [최신 릴리스](https://github.com/ai-campus-kr/shin-player/releases/tag/v0.6.0-beta.3) · [오류 제보와 기능 제안](https://github.com/ai-campus-kr/shin-player/issues)
