...
**YouTube の倍速は保存する GIF に適用されます。キャプチャ自体には元の区間と同じ時間がかかります。**

キャプチャ中は YouTube ウィンドウを表示したままにし、シーク・サイズ変更を避けてください。広告・ライブ配信・保護された動画は非対応です。滑らかさは PC のキャプチャ速度に依存します。

<p align="center"><img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v0.6.0-beta.4/docs/screenshots/v0.6.0-beta.4/01-gif-speed.png" width="620" alt="実際の GIF 作成画面：区間・サイズ・フレーム数と保存結果"></p>

*合成テスト動画を使った実際のアプリ画面です。*

## 使い始める

1. [最新リリース](https://github.com/ai-campus-kr/shin-player/releases/tag/v0.6.0-beta.4)から `ShinPlayer-バージョン-win-x64.zip` をダウンロードし、すべて展開します。
...
**「この内容を説明して」→ 字幕に基づいて回答。「その部分へ移動して」→ 再生位置を移動。** [ベータ版をダウンロード](https://github.com/ai-campus-kr/shin-player/releases/tag/v0.6.0-beta.4)
...
**ベータ版の検証範囲：** 84 項目の統合検査と実際の `gpt-5.4-mini` API 検査を通過しました。実際の YouTube 動画から韓国語字幕 301 件を読み込み、通常の質問では再生位置を維持し、「その部分へ移動して」で追加 API 呼び出しなしに 10:45 へ移動することを確認しました。字幕の有無、広告、ログイン、サイト変更により利用できない動画もあります。
...
0.6 ベータ版は WPF/libmpv と隔離された WebView2 の **84項目の統合検査**で検証します。API は模擬応答を使い、実サービスの検証範囲は上記のとおりです。[変更履歴](CHANGELOG.md)
...
統合検査は計84項目です。シークは2種類のウィンドウサイズと再生リストの状態にわたる28地点、レイアウト更新前の連続入力、マウスを離す位置、キャプチャ解除、ファイル切り替え、全画面を含みます。1〜2倍の WPF レイアウト変換も確認しますが、物理マウス入力や実モニターの DPI 変更は自動化していません。4種類の UI の起動画面・最小サイズ・再生リスト・キャプチャ画面をレンダーし、ボタンの重なりと字幕選択表示を確認します。設定の保存形式と旧設定との互換性、選択ボタン、再生中のスタイル変更に伴う位置・速度の保持も対象です。
...
[한국AI교육진흥원](https://github.com/ai-campus-kr) · [最新リリース](https://github.com/ai-campus-kr/shin-player/releases/tag/v0.6.0-beta.4) · [不具合報告・機能の提案](https://github.com/ai-campus-kr/shin-player/issues)
