...
3. **API 설정** ウィンドウで自分の OpenAI キーを保存します。緑色の **✓ API 키 저장됨** が保存済みの目印です。**연결 확인**（接続確認）で実際の API を試し、成功すると **✓ API 연결 확인됨** と表示します。通常の質問では再生位置を変えず、**「その部分へ移動して」**または根拠の時刻ボタンで移動します。
