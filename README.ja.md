...
**v1.1：** GIF・ショート制作、元動画の領域指定、韓国語フォント6種類、自由な文字色に対応。
...
<p align="center">
  <img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v1.0.0/docs/screenshots/v1.0.0/04-home.png" width="1000" alt="右下に鮮やかな紫のKAI・緑のNAVERバッジを固定した新プレーヤー">
</p>
...
ローカル動画を開いて **쇼츠**（ショート）をクリック。区間選択・縦画面の構図調整・文字入れ・保存まで、プレーヤー内で行えます。

<p align="center"><img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v1.1.0/docs/screenshots/v1.1.0/01-shorts-styles.png" width="980" alt="実際のショート編集画面：縦型プレビュー、ドラッグによる区間選択、韓国語テキストと構図調整"></p>

1. タイムライン両端のハンドルで開始・終了を設定し、中央をドラッグして区間全体を移動します。**장면 미리보기**（フレームプレビュー）で選択区間の画面を確認できます。
2. **구간·화면**（区間・画面）の **영역 직접 선택**（領域を選択）で元動画に長方形を描きます。内側をドラッグして移動し、辺や角で大きさを変更できます。選択直後は領域全体を表示。**100〜400%の拡大**、プレビューのドラッグ、マウスホイールで構図を調整し、リセットで元に戻せます。同じ領域と構図が選択した区間全体に適用されます。
3. **문구·스타일**（文字・スタイル）で最大 **160文字・4行**を入力。Pretendard、Black Han Sans、Do Hyeon、Jua、Nanum Myeongjo、Gaeguの韓国語フォント6種類を同梱しています。別途インストールは不要で、OFLによる商用利用が可能です。**18色のパレット・色選択ダイアログ・HEX入力**、大きさと暗い背景を設定し、文字をドラッグして配置します。
4. **쇼츠 저장**（ショートを保存）で **9:16・1080×1920 / 720×1280・30fps の H.264 MP4** を保存。**원본 소리 포함**をオフにすると音声なしになります。

**保存後は蛍光ライム色の「✓ 쇼츠 저장 완료」（ショート保存完了）が表示され続けます。** ファイル名・長さ・容量を確認し、**완성된 영상 열기**（完成した動画を開く）または **저장 폴더**（保存フォルダー）を選びます。区間・文字・構図を変えると **설정 변경 · 다시 저장 필요**（設定変更・再保存が必要）に切り替わり、前のファイルと区別できます。

- 区間は **0.2秒〜3分**。Windows の **ビデオ → 신플레이어 쇼츠** に保存し、元動画や既存の書き出し結果は上書きしません。
- **ローカル動画専用**。字幕・API キーは不要で、GPT トークンも使いません。元の再生位置と速度は保持します。
- 4種類の UI に対応。キャンセルやウィンドウを閉じた際は変換を停止し、未完成ファイルを削除します。

**[この画面から実際に書き出した MP4 を見る](https://github.com/ai-campus-kr/shin-player/releases/download/v1.1.0/ShinPlayer-shorts-demo.mp4)** · オリジナルの合成デモ動画を使った実際の画面と出力です。

## 好きな場面を GIF に

ローカル動画ファイルを開き、プレーヤーの **GIF** ボタンから、**タイムライン両端のハンドルをドラッグ**して区間を選び、**GIF 만들기**（GIF 作成）を押します。選択範囲の中央をドラッグすると、長さを保ったまま区間を移動できます。

**蛍光ライム色の「✓ GIF 저장 완료」（GIF 保存完了）** と **완성된 GIF 열기**（完成した GIF を開く）で完了を確認できます。完了表示はスクロール領域の外に固定され、自動では消えません。保存中・キャンセル・失敗・設定変更も区別して表示します。
...
<p align="center"><img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v1.0.0/docs/screenshots/v1.0.0/02-gif-completed.png" width="620" alt="実際の GIF 画面：ドラッグで区間選択・移動、拡大と時刻入力の連動"></p>

*合成テスト動画を使った実際のアプリ画面です。*

## 使い始める

1. [最新リリース](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.1.0)から `ShinPlayer-バージョン-win-x64.zip` をダウンロードし、すべて展開します。
...
**「この内容を説明して」→ 字幕に基づいて回答。「その部分へ移動して」→ 再生位置を移動。** [v1.1 をダウンロード](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.1.0)
...
**検証範囲：** 98 項目の統合検査でキーの保存・接続状態、字幕の自動取得・欠落・動画切り替え、チャット配置とローカル GIF を確認しました。実際の YouTube 字幕取得、GPT-5.4-mini の回答・時刻移動はベータ開発時に別途検証しました。v1.1 の回帰検査には模擬 API 応答と隔離された WebView2 ページを使用しました。字幕の提供状況やサイト変更により利用できない動画もあります。
...
v1.1 は WPF/libmpv と隔離された WebView2 の **98項目の統合検査**で検証します。API は模擬応答を使い、実サービスの検証範囲は上記のとおりです。[変更履歴](CHANGELOG.md)
...
統合検査は計98項目です。シークは2種類のウィンドウサイズと再生リストの状態にわたる28地点、レイアウト更新前の連続入力、マウスを離す位置、キャプチャ解除、ファイル切り替え、全画面を含みます。1〜2倍の WPF レイアウト変換も確認しますが、物理マウス入力や実モニターの DPI 変更は自動化していません。4種類の UI の起動画面・最小サイズ・再生リスト・キャプチャ画面をレンダーし、ボタンの重なりと字幕選択表示を確認します。設定の保存形式と旧設定との互換性、選択ボタン、再生中のスタイル変更に伴う位置・速度の保持も対象です。
...
[한국AI교육진흥원](https://github.com/ai-campus-kr) · [最新リリース](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.1.0) · [不具合報告・機能の提案](https://github.com/ai-campus-kr/shin-player/issues)
