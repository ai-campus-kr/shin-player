...
**v1.0 正式リリース：** 常時表示のアドレスバー・動画横の AI チャット・依頼したときに該当区間へ移動。
...
<p align="center"><img src="https://raw.githubusercontent.com/ai-campus-kr/shin-player/v1.0.0/docs/screenshots/v1.0.0/01-shorts-completed.png" width="980" alt="実際のショート編集画面：縦型プレビュー、ドラッグによる区間選択、韓国語テキストと構図調整"></p>

1. タイムライン両端のハンドルで開始・終了を設定し、中央をドラッグして区間全体を移動します。**장면 미리보기**（フレームプレビュー）で選択区間の画面を確認できます。
2. **세로로 꽉 채우기**（縦画面を満たす）では画像をドラッグして構図を調整。**원본 전체 보이기**（元の画面全体を表示）では黒い余白を入れ、画面を切り取りません。
3. **올릴 문구**（追加する文字）に最大 **160文字・4行**を入力。大きさ、白・黄・ミント、暗い背景を選び、プレビュー上で文字をドラッグして配置します。
...
**[この画面から実際に書き出した MP4 を見る](https://github.com/ai-campus-kr/shin-player/releases/download/v1.0.0/ShinPlayer-shorts-demo.mp4)** · オリジナルの合成デモ動画を使った実際の画面と出力です。
...
1. [最新リリース](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.0.0)から `ShinPlayer-バージョン-win-x64.zip` をダウンロードし、すべて展開します。
...
**「この内容を説明して」→ 字幕に基づいて回答。「その部分へ移動して」→ 再生位置を移動。** [v1.0 をダウンロード](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.0.0)
...
**検証範囲：** 95 項目の統合検査でキーの保存・接続状態、字幕の自動取得・欠落・動画切り替え、チャット配置とローカル GIF を確認しました。実際の YouTube 字幕取得、GPT-5.4-mini の回答・時刻移動はベータ開発時に別途検証しました。v1.0 の回帰検査には模擬 API 応答と隔離された WebView2 ページを使用しました。字幕の提供状況やサイト変更により利用できない動画もあります。
...
v1.0 は WPF/libmpv と隔離された WebView2 の **95項目の統合検査**で検証します。API は模擬応答を使い、実サービスの検証範囲は上記のとおりです。[変更履歴](CHANGELOG.md)
...
統合検査は計95項目です。シークは2種類のウィンドウサイズと再生リストの状態にわたる28地点、レイアウト更新前の連続入力、マウスを離す位置、キャプチャ解除、ファイル切り替え、全画面を含みます。1〜2倍の WPF レイアウト変換も確認しますが、物理マウス入力や実モニターの DPI 変更は自動化していません。4種類の UI の起動画面・最小サイズ・再生リスト・キャプチャ画面をレンダーし、ボタンの重なりと字幕選択表示を確認します。設定の保存形式と旧設定との互換性、選択ボタン、再生中のスタイル変更に伴う位置・速度の保持も対象です。
...
[한국AI교육진흥원](https://github.com/ai-campus-kr) · [最新リリース](https://github.com/ai-campus-kr/shin-player/releases/tag/v1.0.0) · [不具合報告・機能の提案](https://github.com/ai-campus-kr/shin-player/issues)
