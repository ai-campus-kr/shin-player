...
internal sealed class SubtitleCaptureWindow : Window
{
    ...
    internal SubtitleCaptureWindow(string path, double delay)
    {
        ...
        var filename = new TextBlock { Text = Path.GetFileName(path), TextTrimming = TextTrimming.CharacterEllipsis, ToolTip = path, Margin = new(0, 8, 0, 20) };
        filename.SetResourceReference(TextBlock.ForegroundProperty, "Muted"); heading.Children.Add(filename);
        ...
        var description = new TextBlock { Text = "각 구간의 중간 시점을 PNG로 저장합니다. 재생 위치는 바뀌지 않습니다.\nGPT/API를 사용하지 않습니다. 외부·이미지 자막은 지원하지 않습니다.", TextWrapping = TextWrapping.Wrap, FontSize = 12, Margin = new(0, 14, 0, 0) };
        description.SetResourceReference(TextBlock.ForegroundProperty, "Muted"); body.Children.Add(description);
